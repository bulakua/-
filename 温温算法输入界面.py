...
spots=['stadium','auditorium','library','lecturehall','clinicbuilding',
      'laboratorybuilding','firstdininghall','teachingbuilding',
      'officebuilding','gogonewworld','octagon','tunnel',
      'seconddininghall','youkangmarket','dorm']
window = tk.Tk()
...
label.pack()
entry = tk.Entry(window,show=None,bg='white')
entry.pack()

# label_1 = tk.Label(window,text='Please Input Your Destination:')
# label_1.pack()
# entry_1 = tk.Entry(window,show=None,bg='white')
# entry_1.pack()
...
def Answer():
    ...
    filename="D:/Practice Week/FDfsFind.txt"
    if start in spots :# 检验输入
        start = start + "\n"
        save(filename,start)
        window.destroy()
    else:
...
b.pack()
t = scrolledtext.ScrolledText(window,height=4,bg='white')
